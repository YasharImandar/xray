...
_openapi:
  ...
  toc:
    ...
    - depth: 2
      title: 'Live traffic snapshot for the inbound named extension (port 2053):
        parsed access-log events, per-client last destination, and window
        totals.'
      url: '#live-traffic-snapshot-for-the-inbound-named-extension-port-2053-parsed-access-log-events-per-client-last-destination-and-window-totals'
    - depth: 2
      title: Truncate the Xray access log the Monitoring page reads. New extension
        traffic is recorded again immediately. Does not change inbound traffic
        counters.
      url: '#truncate-the-xray-access-log-the-monitoring-page-reads-new-extension-traffic-is-recorded-again-immediately-does-not-change-inbound-traffic-counters'
    - depth: 2
  ...
  structuredData:
    headings:
      ...
      - content: 'Live traffic snapshot for the inbound named extension (port 2053):
          parsed access-log events, per-client last destination, and window
          totals.'
        id: live-traffic-snapshot-for-the-inbound-named-extension-port-2053-parsed-access-log-events-per-client-last-destination-and-window-totals
      - content: Truncate the Xray access log the Monitoring page reads. New extension
          traffic is recorded again immediately. Does not change inbound traffic
          counters.
        id: truncate-the-xray-access-log-the-monitoring-page-reads-new-extension-traffic-is-recorded-again-immediately-does-not-change-inbound-traffic-counters
