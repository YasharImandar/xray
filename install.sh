...
# Prebuilt release tarballs and scripts are fetched from this GitHub repo.
# Override with XUI_GH_REPO if you are mirroring the fork.
xui_gh_repo="${XUI_GH_REPO:=YasharImandar/xray}"
...
resolve_latest_tag() {
    local url tag
    url=$(curl -sSLI -o /dev/null -w '%{url_effective}' --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 60 "https://github.com/${xui_gh_repo}/releases/latest" 2>/dev/null)
    tag=${url##*/tag/}
    if [[ "$tag" != "$url" && -n "$tag" && "$tag" != "latest" ]]; then
        echo "$tag"
        return 0
    fi
    curl -Ls --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 60 "https://api.github.com/repos/${xui_gh_repo}/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/'
}
...
require_repo_files() {
    ...
    [[ "${ref}" == "main" ]] && return 0
    for name in "$@"; do
        status=$(curl -sIL --retry 3 --connect-timeout 15 -o /dev/null -w '%{http_code}' "https://raw.githubusercontent.com/${xui_gh_repo}/${ref}/${name}")
        if [[ "${status}" != "200" ]]; then
...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        # This fork ships a rolling per-commit binary as tag `dev-latest`.
        # A one-line install with no version argument always gets that build.
        tag_version="dev-latest"
        echo -e "Got x-ui latest version: ${tag_version}, beginning the installation..."
        curl -fLR --retry 5 --retry-delay 3 --connect-timeout 15 --speed-limit 1 --speed-time 300 -o ${xui_folder}-linux-$(arch).tar.gz https://github.com/${xui_gh_repo}/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        verify_release_checksum "https://github.com/${xui_gh_repo}/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz" "${xui_folder}-linux-$(arch).tar.gz"
    else
        tag_version=$1
        ...
        url="https://github.com/${xui_gh_repo}/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
    ...
    curl -fLRo "${xui_script_temp}" "https://raw.githubusercontent.com/${xui_gh_repo}/${script_ref}/x-ui.sh"
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        curl -fLRo "${xui_rc_temp}" "https://raw.githubusercontent.com/${xui_gh_repo}/${script_ref}/x-ui.rc"
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    service_unit_url="https://raw.githubusercontent.com/${xui_gh_repo}/${script_ref}/x-ui.service.debian"
                    ;;
                arch | manjaro | parch)
                    service_unit_url="https://raw.githubusercontent.com/${xui_gh_repo}/${script_ref}/x-ui.service.arch"
                    ;;
                *)
                    service_unit_url="https://raw.githubusercontent.com/${xui_gh_repo}/${script_ref}/x-ui.service.rhel"
                    ;;
            esac
